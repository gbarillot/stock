...
describe Position do
  ...
  describe "#link" do
    ...
    #   assert_equal $current_user.id, history.user_id
    #   assert_equal Position.last.id, history.item_id
    #   assert_equal "Position", history.item_type
    #   assert_equal ({"add"=>2, "product_id"=>products(:product_1).id}), history.actions
    # end
    #
    ...
    #   assert_equal 1, Position.all.count
    #   assert_equal 0, History.all.count
    # end
    #
  ...
  describe "#unlink" do
    it "fails cause not enough products" do
      pos = Position.link(product_id: products(:product_1).id, quantity: 10, name: 'a b 2 5')
      pos.unlink(11)

      assert_equal 10, pos.quantity
      assert_equal 1, History.all.count
      assert_equal ({:quantity=>["can't be greater"]}), pos.errors.messages
    end

    it "decrement quantity for a given position" do
      pos = Position.link(product_id: products(:product_1).id, quantity: 10, name: 'a b 2 5')
      pos.unlink(4)

      assert_equal 6, pos.quantity
      assert_equal 2, History.all.count
      assert_equal ({"remove"=>4, "product_id"=>products(:product_1).id}), History.last.actions
    end

    it "decrement quantity inside the whole tree" do
      pos = Position.link(product_id: products(:product_1).id, quantity: 10, name: 'a b 2 5')
      pos.unlink(4)
      ...
      assert_equal 6, l1.quantity
      assert_equal 6, l2.quantity
      assert_equal 6, l3.quantity
      assert_equal 6, l4.quantity
