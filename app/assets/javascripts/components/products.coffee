class @Products
  ...
  @add = (product) ->
    ...
    out += "<h4 class='card-title'><a href='/products/#{product.id}'>#{product.reference}</a></h4>"
    out += "<hr>"
    out += "<p class='card-text'>"
    if product.name != ''
      out += "Nom: <a href='/products/#{product.id}'>#{product.name}</a>"
      out += "<br>"
    out += "Emplacement: <a href='/positions/#{product.position}'>#{product.position}</a>"
    out += "<br>"
    out += "Stock: <strong>#{product.quantity}</strong>"
    out += "</p>"
    out += "</div>"
    out += "</div>"
    out += "</div>"
